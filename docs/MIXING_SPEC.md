...
| Focus `mono_compat` (Step 1) | L/R cross-spectrum per voice; `loss_db=10log10((Σ\|L\|²+Σ\|R\|²+2ΣReLR)/(Σ\|L\|²+Σ\|R\|²))`, coherence-gated | computeMonoCompat | **§14.9 BUILT** — within-track mono cancellation. Hard-pan reads ~0 (not fake cancel). |
| Focus `phase_cancellation` (Step 2) | between-voice cross-spectrum on mono M; `cancel_db`, `phase_deg`, `coherence` per 1/6-oct bin, per unordered pair | computeBetweenPhase | **§14.9 BUILT 2026-06-08** — the genuinely new tool (masking can't see cancellation). Gates: presence + γ²_corr>0.5 + cancel<−1.5dB + n_eff≥8. All pairs (kick/bass any voice). Delay-comp (C4) deferred → delayed pairs abstain (safe). |
...
- ✅ **BUILT — Step 2: between-element phase cancellation (§14.3, the genuinely new tool).** Max: each `spectrum_analyzer_focus_N.maxpat` now poke~s its complex MID spectrum (mid real = `obj-9` out, mid imag = `obj-10` out, fft index = `fftin~ 1` out 2) into `focus_N_reM`/`focus_N_imM` (buffers added to `focus_chain_N.maxpat`). JS: `snapshot_focus_spectrum` reads each voice's complex mid per tick into `tickMid`, accumulates per-voice `focusMidPow` (Σ|M|²) and, for every co-occurring voice PAIR, `focusPairAccum["lo_hi"]` = ΣRe(M_i·conj M_j) / ΣIm. `computeBetweenPhase(lo,hi,frames)` aggregates per 1/6-oct focus bin → `cancel_db = 10·log10((Σ|A|²+Σ|B|²+2ΣRe)/(Σ|A|²+Σ|B|²))`, coherence `γ²=(ΣRe²+ΣIm²)/(Σ|A|²Σ|B|²)` (bias-corrected, n_eff-gated ≥8 per **C2**), `phase_deg=atan2(ΣIm,ΣRe)`. **Gates (foolproof):** both voices present (−30 dB floor), `γ²_corr>0.5` (the safeguard — also auto-abstains on unknown-delay pairs, see note), `cancel_db<−1.5`. `analyzeFocus` runs ALL unordered pairs (kick/bass may be in any voice), skips ancestor/descendant pairs, emits `phase_cancellation = {"A↔B": [{hz,cancel_db,phase_deg,coherence}]}`. Passthrough: tools.js → compress.js → prompt. **User must SAVE the device in the Max editor then reload.**
  - **Wiring choice (deviates from the original §14.3 plan, deliberately):** cross-spectrum is accumulated **in JS, pairwise**, not in Max DSP per pair. Buffer reads are in-process (no Max→browser bridge), so the bridge-drop argument for Max-side accumulation does not apply here; JS pairing needs only **2 extra poke~ per voice** (O(N)) instead of O(N²) Max pair-accumulator chains, and keeps the heavy math flexible. Same architecture as Step 1 mono-compat.
  - **C4 (inter-tap delay removal) deferred — and that is SAFE, not a gap.** No explicit τ estimation yet. An unknown delay rotates phase across frequency, which **collapses coherence** → the γ²>0.5 gate makes the pair abstain rather than report a phantom. So v1 is conservative (never lies; may stay silent on a genuinely-delayed pair that a delay-comp would recover). Matches the prime directive. Add cross-correlation-lag delay-comp later to recover those abstained pairs.
  - **Mid-sum approximation:** cross-spectrum is on each voice's mono M=(L+R)/2, not L/R separately. Exact for the headline mono low-end (kick/sub); a reasonable approximation for wide stereo content. L+R-separate is a future refinement if wide-content phase proves important.
